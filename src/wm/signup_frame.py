from tkinter import Frame, Label, Entry, Button, Checkbutton, messagebox, PhotoImage
import pymysql
...
class SignupFrame(Frame):
    ...
    def create_widgets(self):
        ...
        Label(self.confirm_pass_frame, text="Confirm Password:", bg="#82AACF").pack(side="left")
        self.confirm_pass_entry = Entry(self.confirm_pass_frame, width=30, show="*")
        self.confirm_pass_entry.pack(side="left", padx=10)

        # Create a button to toggle password visibility
        self.show_confirm_pass_button = Button(self.confirm_pass_frame, image=self.show_pass_image, bg="#82AACF", bd=0, command=self.toggle_confirm_password)
        self.show_confirm_pass_button.pack(side="left", padx=5)

        # Create a button to submit data
        self.submit_button = Button(self, text='Submit', bg='white', width=15, borderwidth=1, height=2, command=self.submit)
    ...
    def submit(self):
        ...
        if self.user_entry.get() == '':
            ...
            db = pymysql.connect(host='localhost', user='root', password='wellnessmate1234')
            try:
                cur = db.cursor()
