...
class MyHabits(Frame):
    ...
    def mount_habit_creator(self):
        # Create the habit creator frame
        habit_creator_frame = Frame(self, bg="#93CE82")
        habit_creator_frame.grid(row=0, column=0, sticky='nswe', pady=20, padx=20)
        habit_creator_frame.grid_columnconfigure(0, weight=1)        

        # Create and add widgets to the habit creator frame
        label = Label(habit_creator_frame, text=f"Habit Creator", font=("Helvetica", 20), bg="#93CE82", fg="black")
    ...
    def mount_habit_list(self):
        # Create the habit list frame
        self.habit_list_frame = Frame(self, bg="#D9EAD3")
        self.habit_list_frame.grid(row=1, column=0, sticky='nswe', pady=20, padx=20)
        self.habit_list_frame.grid_columnconfigure(0, weight=1)

        label = Label(self.habit_list_frame, text=f"Habit List", font=("Helvetica", 20), bg="#D9EAD3", fg="black")
        label.grid(row=0, column=0, sticky='nsew', padx=10, pady=20)  
        label.grid_columnconfigure(0, weight=1)   

        self.empty_list_label = Label(self.habit_list_frame, text=f"You don't have any habit yet.", font=("Helvetica", 14), bg="#D9EAD3", fg="black")
