...
class Database:
    def __init__(self):
        ...
        try:
            ...
            self.db.close()
            self.db = pymysql.connect(
                host=self.__host, user=self.__user, password=self.__password, database="wm_db"
            )
    ...
    def create_database(self):
        try:
            self.cur.execute("CREATE SCHEMA IF NOT EXISTS `wm_db` DEFAULT CHARACTER SET utf8;")
            self.db.commit()
